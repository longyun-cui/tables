...
<div class="box-body chart-container chart-option">
    ...
    <form action="" method="post" class="form-horizontal form-bordered form-edit-column">
        {{csrf_field()}}
        <input type="hidden" readonly name="operate" value="edit">
        <input type="hidden" readonly name="container" value="column">
        <input type="hidden" readonly name="chart_id" value="{{$chart_encode or encode(0)}}">
        ...
        <div class="form-group">
            <label class="control-label col-md-2">图类型</label>
            <div class="col-md-8">
                <select class="form-control chart-type" name="type" onchange="select_chart_type()">
                    <option value="1" @if($data->type == 1) selected="selected" @endif>折线图</option>
                    <option value="2" @if($data->type == 2) selected="selected" @endif>柱状图</option>
                    <option value="3" @if($data->type == 3) selected="selected" @endif>饼图</option>
                    <option value="4" @if($data->type == 4) selected="selected" @endif>雷达图</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-8">
                <div><input type="text" class="form-control" name="name" placeholder="请输入后台名称" value="{{$data->name or ''}}"></div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-8">
                <div><input type="text" class="form-control" name="description" placeholder="请输入备注" value="{{$data->description or ''}}"></div>
            </div>
        </div>
